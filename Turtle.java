...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a)
    {
        ...
        Turtle mek = new Turtle();
        Turtle fah = new Turtle();
        double amplitude = 10;
        ...
        for(int j = 0; j < 50; j++){
            fah.up();
            fah.setPosition(0,0-(j*10));
            fah.backward(length / 2);
            fah.down();
            for (int x = 0; x <= length; x++) {

                // Calculate y-coordinate based on sine function
                double y = amplitude * Math.sin(frequency * x);

                // Move turtle to the calculated point
                fah.setPosition(x - (length / 2), y-(j*10));
            }
